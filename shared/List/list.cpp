...
int list_verify(const List* list) {
    ...
    ssize_t log_i = 0;
    LIST_FOREACH(*list, phys_i, log_i) {
        CHECK_ERR_(list->arr[phys_i].elem.type == IRNodeType::DEFAULT, list->POISON_VAL_FOUND);
    ...
    for (phys_i = 1; phys_i < list->capacity; phys_i++) {
        CHECK_ERR_(list->arr[phys_i].prev != -1 && list->arr[phys_i].elem.type == IRNodeType::DEFAULT,
...
int list_insert_after(List* list, const size_t position, const ListElem_t elem, size_t* inserted_index) {
    int res = LIST_ASSERT(list);
    ...
    size_t new_i = (size_t)list->free_head;
    list->free_head = list->arr[new_i].next;

    list->arr[new_i].prev = (ssize_t)position;
    list->arr[new_i].next = list->arr[position].next;

    list->arr[list->arr[position].next].prev = (ssize_t)new_i;
    list->arr[position].next = (ssize_t)new_i;

    list->arr[new_i].elem = elem;

    list->size++;

    if ((ssize_t)new_i != list_tail(list) && (ssize_t)new_i != list_head(list))
        list->is_linear = false;

    if (inserted_index != nullptr) *inserted_index = new_i;
