...
Фрагмент описания грамматики языка:
...
Делается один проход оптимизации. Каждая операция `push` помещается в стек. При встрече `pop` происходит проверка: не меняются ли какие-либо данные между текущей операцией и последним `push`. Если нет, то эти две операции либо уничтожаются, либо заменяются на одну.
...
Из линейного IR достаточно тривиально реализуется генерация ассемблерного текста программы. В IR все прыжки и вызовы функций ссылаются на другие блоки IR по их индексам. Так удаётся переложить проблему расчёта адресов на ассемблер. В дальнейшем при генерации бинарного исполняемого файла будут реализованы фиксапы.
