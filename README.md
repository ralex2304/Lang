...
Компиляция программ на своём Тьюринг-полном эзотерическом языке PPLang для [собственного `SPU`](https://github.com/ralex2304/Processor) и `x86-64`.
...
1. [Процесс компиляции](#процесс-компиляции)
    1. [Frontend](#frontend)
        1. [Лексер](#лексер)
        2. [Синтаксический парсер](#синтаксический-парсер)
    ...
    4. [IR Backend](#ir-backend)
        1. [Оптимизации IR](#оптимизации-ir)
        2. [Генерация ассемблерного текста](#генерация-ассемблерного-текста)
        3. [ABI для `x86-64`](#abi-для-x86-64)
        4. [ABI для `SPU`](#abi-для-spu)
        5. [Библиотека функций ввода и вывода для `x86-64`](#библиотека-функций-ввода-и-вывода-для-x86-64)
        6. [Парсинг объектного файла библиотеки ввода и вывода](#парсинг-объектного-файла-библиотеки-ввода-и-вывода)
        7. [Генерация исполняемого файла `ELF64`](#генерация-исполняемого-файла-elf64)
...
3. [Примеры программ](#примеры-программ)
    1. [Рекурсивный расчёт факториала](#рекурсивный-расчёт-факториала)
        1. [язык Пророка Санбоя](#язык-пророка-санбоя)
        2. [`C`-подобные ключевые слова](#c-подобные-ключевые-слова)
    2. [Демонстрация работы областей видимости переменных](#демонстрация-работы-областей-видимости-переменных)
        1. [`C`-подобные ключевые слова](#c-подобные-ключевые-слова)
4. [Сравнение эффективности скомпилированных программ](#сравнение-эффективности-скомпилированных-программ)
    1. [Программа на PPLang](#программа-на-pplang)
    ...
    4. [Измерение эффективности](#измерение-эффективности)
...
8. [Итоги](#итоги)
9. [Источники и инструменты](#источники-и-инструменты)
...
Здесь оптимизируется последовательность блоков IR и генерируется ассемблерный текст или исполняемый файл для двух архитектур: `x86-64` и [`SPU`](https://github.com/ralex2304/Processor).
...
![Сравнение IR с оптимизациями и без](img/ir_opt_comparison.png)

Прирост быстродействия программы на `x86-64` составил порядка 25%. [Подробное исследование](#измерение-эффективности) ниже.
...
Библиотека написана на ассемблере и содержит наработки из проекта [AsmPrintf](https://github.com/ralex2304/x86AsmCourse). [Текст библиотеки](Programs/doubleiolib.nasm).
...
<img rel="elf64 creation" src="img/elf64_creation.png" width="50%">
...
Для упрощения отладки и верификации генерации бинарных команд был реализован [DSL](ir_backend/src/asm/x86_64/opcodes.h). Так же были написаны функции для автоматической генерации полей `ModRM` и `SIB`.
...
<pre>
<b><имя> var</b> <i>const</i> <i>= <выражение></i>
---
x var
---
y var = 123
...
- Объявление массива:
<pre>
<b><имя> var</b> <i>const</i> <b>[</b><i><размер></i><b>]</b> <i>= <значение 1>, <значение 2>, ...</i>
---
x var [3]
---
y var [] = 1, 2, 3
...
- Объявление функции:
<pre>
<b><имя> var (</b><i><аргументы через запятую (объявления переменных)></i><b>) {
    <тело функции>
}</b>
---
func1 var () {
...
- Вызов функции:
<pre>
<b><имя>(</b><i><выражения через запятую></i><b>)</b>
---
func1()
---
x = func2(1, 2)
...
>
> В папке есть небольшой [readme](Programs/README.md)
...
![factorial AST](img/fact_ast.png)
...
3. [`bear`](https://github.com/rizsotto/Bear) - *необязательно (`make build nobear=1`)* - утилита для создания файла `compile_commands.json` (нужен для языкового сервера `clangd`)
